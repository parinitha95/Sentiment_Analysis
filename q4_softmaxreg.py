...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    mat=np.zeros((len(sentence),wordVectors.shape[1]))
    ### YOUR CODE HERE
    cnt=0
    for s in sentence:
        mat[cnt,:]=wordVectors[tokens[s],:]
        cnt+=1
    sentVector=np.sum(mat,axis=0)/len(sentence)
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    #        np.argmax helpful)  
    #print labels, weights.shape,features.shape
